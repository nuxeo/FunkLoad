...
#
def get_version():
    """Return the FunkLoad package version."""
    # NOTE(mattv): The following line is changed from the original version of
    # Funkload.  Since the original version of FunkLoad uses setuptools, it
    # previously called a function from pkg_resources in setuptools that is not
    # useful here.
    import version
...
def has_method(instance, method_name):
    """Determine if an instance has a method with the given name.

    Args:
      instance: an instance of a class
      method_name: name of a method to find.

    Returns:
      True if the method exists in the instance, False otherwise.
    """
    if (hasattr(instance, method_name) and
        type(getattr(instance, method_name)) == types.MethodType):
        return True
    else:
        return False
