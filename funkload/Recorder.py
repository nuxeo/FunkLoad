...
"""
    def __init__(self, argv=None):
        ...
        self.temp_suffix = '_funkload'
        self.tcpwath_syspath = 'tcpwatch.py'
        self.parseArgs(argv)
    ...
    def startProxy(self):
        ...
        self.tcpwatch_path = mkdtemp(self.temp_suffix)
        cmd = '%s -p %s -s -r %s' % (self.tcpwatch_syspath,
                                     self.port,
                                     self.tcpwatch_path)
        if self.verbose:
            cmd += ' | grep "T http"'
    ...
    def reindent(self, code, indent=None):
        """Improve indentation."""
        if indent is None:
           indent = self.indent
        spaces = ' ' * indent
