...
class LoopTestRunner(threading.Thread):
    ...
    def __init__(self, test_module, test_class, test_name, options,
                 cycle, cvus, thread_id, sleep_time):
...
class BenchRunner:
    ...
    def __init__(self, module_file, class_name, method_name, options):
        ...
        fconf = test.fconf
        self.config_path = fconf.config_path
        self.result_path = fconf.result_path
        self.class_title = fconf.get('title', section='main')
        self.class_description = fconf.get('description', section='main')
        self.test_id = self.method_name
        self.test_description = fconf.get('description',
                                          'No test description',
                                          section=self.method_name)
        self.test_url = fconf.get('url', section='main')
        self.cycles = map(int, fconf.getList('cycles', section='bench'))
        self.duration = fconf.getFloat('duration', section='bench')
        self.startup_delay = fconf.getFloat('startup_delay',
                                            section='bench')
        self.cycle_time = fconf.getFloat('cycle_time', section='bench')
        self.sleep_time = fconf.getFloat('sleep_time', section='bench')
        self.sleep_time_min = fconf.getFloat('sleep_time_min',
                                             section='bench')
        self.sleep_time_max = fconf.getFloat('sleep_time_max',
                                             section='bench')

        # setup monitoring
        monitor_hosts = []                  # list of (host, port, descr)
        for host in fconf.get('hosts', '', quiet=True,
                              section='monitor').split():
            host = host.strip()
            monitor_hosts.append((host, fconf.getInt('port', section=host),
                                  fconf.get('description', '', section=host)))
    ...
    def logr(self, message):
        ...
        self.test.logger.results.raw_write(message)
    ...
    def logr_open(self):
        ...
        for (host, port, desc) in self.monitor_hosts:
            config[host] = desc
        self.test.logger.results.write_header(**config)

    def logr_close(self):
        """Stop logging tag."""
        self.test.logger.results.close()
...
def main():
    ...
    parser.add_option("--no-strict-cookies", dest="strict_cookies",
                      action="store_false", default=True,
                      help="Disable strict handling of cookie domains.")
    parser.add_option("--fetch-threads", type="int", default=4,
                      help="How many image and link fetching threads "
                      "to start.")
