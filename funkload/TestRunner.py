...
from funkload.FunkLoadBaseTestCase import FunkLoadBaseTestCase
...
class TestLoader(unittest.TestLoader):
    ...
    def loadTestsFromTestCase(self, testCaseClass):
        ...
        if not issubclass(testCaseClass, FunkLoadBaseTestCase):
    ...
    def loadTestsFromName(self, name, module=None):
        ...
        if type(obj) == types.ModuleType:
            ...
            if issubclass(obj.im_class, FunkLoadBaseTestCase):
...
"""
    ...
    def parseArgs(self, argv):
        ...
        if self.module is None:
            ...
            # remove the .py
            module = args[0]
            if module.endswith('.py') or module.endswith('.par'):
        ...
        if not getattr(options, "verbosity", None):
            options.verbosity = FunkLoadLogger.INFO
            if options.quiet:
                options.verbosity = FunkLoadLogger.WARN
                g_doctest_verbose = False
            if options.verbose:
                options.verbosity = FunkLoadLogger.VERBOSE
            if options.debug:
                options.verbosity = FunkLoadLogger.DEBUG
            if options.debug_level:
                # translate debug_level into verbosity level for backward
                # compatibility
                level = int(options.debug_level)
                if level >= 2:
                    options.verbosity = FunkLoadLogger.VDEBUG

        # force console logging if debug levels are used
    ...
    def runTests(self):
        ...
        if getattr(self.options, "exit_after_running", True):
